...
def get_similar_repos():
    ...
    #     similar_repos, key=lambda x: x[1], reverse=True)[1:11]
    # result = [{'name': df.loc[i, 'name'], 'similarity_score': score, 'desc':df.loc[i, 'desc'],
    #            'tags':df.loc[i, 'tags'], 'link':df.loc[i, 'upforgrabs__link']} for i, score in sorted_similar_repos]
